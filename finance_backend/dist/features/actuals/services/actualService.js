...
exports.fetchAvailableYears = exports.getCustomers = exports.upsertDepreciation = exports.fetchDepreciation = exports.fetchSummary = exports.upsertBankCharges = exports.fetchBankCharges = exports.upsertSalaries = exports.fetchSalaries = exports.upsertCorporateExpenses = exports.fetchCorporateExpenses = exports.upsertRevenueDirectExpenses = exports.fetchRevenueDirectExpenses = void 0;
const database_1 = require("../../../config/database");
const MONTHS = ['apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec', 'jan', 'feb', 'mar'];
// ==========================================
// 1. REVENUE & DIRECT EXPENSES INTEGRATION
// ==========================================
const fetchRevenueDirectExpenses = async (year) => {
    const [manualRows] = await database_1.db.query('SELECT * FROM actual_revenue_direct_expenses WHERE financial_year = ?', [year]);
    const groupedData = {};
...
exports.fetchRevenueDirectExpenses = fetchRevenueDirectExpenses;
const upsertRevenueDirectExpenses = async (year, groups) => {
    ...
    try {
        ...
        const [existingRows] = await connection.query('SELECT DISTINCT customer, project, location FROM actual_revenue_direct_expenses WHERE financial_year = ?', [year]);
        // 3. Delete groups that are no longer present in incoming data
        for (const row of existingRows) {
            const key = `${row.customer}|||${row.project}|||${row.location}`;
            if (!incomingKeys.has(key)) {
                await connection.query(`DELETE FROM actual_revenue_direct_expenses WHERE financial_year = ? AND customer = ? AND project = ? AND location = ?`, [year, row.customer, row.project, row.location]);
            }
        }
        ...
        for (const group of groups) {
            ...
            for (const month of MONTHS) {
                ...
                await connection.query(`INSERT INTO actual_revenue_direct_expenses 
        ...
        for (const month of MONTHS) {
            ...
            await connection.query(`INSERT INTO actual_revenue_direct_expenses_totals 
         ...
         ON DUPLICATE KEY UPDATE 
          ...
          gross_margin_pct = VALUES(gross_margin_pct)`, [year, month, rev, de, gm, dePct, gmPct]);
        }
        // Save totals for Revenue Direct Expenses
        await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'RevenueDirectExpense', 'Total Revenue', totalRevenue]);
        await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'RevenueDirectExpense', 'Total Direct Expenses', totalDirectExpense]);
...
exports.upsertRevenueDirectExpenses = upsertRevenueDirectExpenses;
// ==========================================
// 2. CORPORATE EXPENSES INTEGRATION
// ==========================================
const fetchCorporateExpenses = async (year) => {
    const [startYearStr, endYearStr] = year.split('-');
    const startYear = parseInt(startYearStr);
    const endYear = parseInt(endYearStr);
    const [rows] = await database_1.db.query('SELECT * FROM actual_corporate_expenses WHERE financial_year = ?', [year]);
    const grouped = {};
    // 1. Load manual heads
    rows.forEach((row) => {
        if (!grouped[row.head]) {
  ...
  `;
    const [hrmsRows] = await database_1.db.query(liveQuery, [startYear, endYear]);
    const monthMap = {
        4: 'apr', 5: 'may', 6: 'jun', 7: 'jul', 8: 'aug', 9: 'sep', 10: 'oct', 11: 'nov', 12: 'dec',
        1: 'jan', 2: 'feb', 3: 'mar'
    };
...
exports.fetchCorporateExpenses = fetchCorporateExpenses;
const upsertCorporateExpenses = async (year, data) => {
    const connection = await database_1.db.getConnection();
    try {
        await connection.beginTransaction();
        let moduleTotal = 0;
        ...
        const [existingRows] = await connection.query('SELECT DISTINCT head FROM actual_corporate_expenses WHERE financial_year = ?', [year]);
        ...
        for (const row of existingRows) {
            if (!incomingHeads.has(row.head) && !row.head.endsWith('(HRMS)')) {
                await connection.query(`DELETE FROM actual_corporate_expenses WHERE financial_year = ? AND head = ?`, [year, row.head]);
            }
        }
        ...
        for (const row of data) {
            ...
            for (const month of MONTHS) {
                ...
                if (!isHrms) {
                    await connection.query(`INSERT INTO actual_corporate_expenses (financial_year, head, month, amount)
             VALUES (?, ?, ?, ?)
            ...
            await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'CorporateExpenses', row.head, headTotal]);
        }
        // Save grand total
        await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'CorporateExpenses', 'Grand Total', moduleTotal]);
        await connection.commit();
...
exports.upsertCorporateExpenses = upsertCorporateExpenses;
// ==========================================
// 3. SALARY INTEGRATION
// ==========================================
const fetchSalaries = async (year) => {
    const [startYearStr, endYearStr] = year.split('-');
    const startYear = parseInt(startYearStr);
    const endYear = parseInt(endYearStr);
    const [manualRows] = await database_1.db.query('SELECT * FROM actual_salaries WHERE financial_year = ?', [year]);
    const grouped = {};
    manualRows.forEach((row) => {
...
const upsertSalaries = async (year, data) => {
    const connection = await database_1.db.getConnection();
    try {
        await connection.beginTransaction();
        let moduleTotal = 0;
        ...
        for (const [head, total] of Object.entries(headTotals)) {
            await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'Salaries', head, total]);
        }
        // Save grand total
        await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'Salaries', 'Grand Total', moduleTotal]);
        await connection.commit();
...
exports.upsertSalaries = upsertSalaries;
// ==========================================
// 4. BANK CHARGES INTEGRATION
// ==========================================
const fetchBankCharges = async (year) => {
    const [rows] = await database_1.db.query('SELECT * FROM actual_bank_charges WHERE financial_year = ?', [year]);
    const grouped = {};
    rows.forEach((row) => {
        if (!grouped[row.head]) {
            grouped[row.head] = { head: row.head, year: row.financial_year };
...
exports.fetchBankCharges = fetchBankCharges;
const upsertBankCharges = async (year, data) => {
    const connection = await database_1.db.getConnection();
    try {
        await connection.beginTransaction();
        let moduleTotal = 0;
        ...
        const incomingHeads = new Set(data.map(row => row.head));
        // 2. Fetch existing heads in DB for this year
        const [existingRows] = await connection.query('SELECT DISTINCT head FROM actual_bank_charges WHERE financial_year = ?', [year]);
        // 3. Delete heads that are no longer present in incoming data
        for (const row of existingRows) {
            if (!incomingHeads.has(row.head)) {
                await connection.query(`DELETE FROM actual_bank_charges WHERE financial_year = ? AND head = ?`, [year, row.head]);
            }
        }
        // 4. Insert or Update incoming heads
        for (const row of data) {
            ...
            for (const month of MONTHS) {
                const amount = row[month] || 0.00;
                await connection.query(`INSERT INTO actual_bank_charges (financial_year, head, month, amount)
           VALUES (?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE amount = VALUES(amount)`, [year, row.head, month, amount]);
                headTotal += amount;
                moduleTotal += amount;
            ...
            // Save individual head total
            await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'BankCharges', row.head, headTotal]);
        }
        // Save grand total
        await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'BankCharges', 'Grand Total', moduleTotal]);
        await connection.commit();
...
exports.upsertBankCharges = upsertBankCharges;
// ==========================================
// 5. SUMMARY CONSOLIDATION & CALCULATIONS
// ==========================================
const fetchSummary = async (year) => {
    // 1. Fetch Revenue & Pct
    const revExpRows = await (0, exports.fetchRevenueDirectExpenses)(year);
    // 2. Fetch Corporate Expenses
    const corpRows = await (0, exports.fetchCorporateExpenses)(year);
    // 3. Fetch Bank Charges
    const bankRows = await (0, exports.fetchBankCharges)(year);
    // 4. Fetch Depreciation/Tax manual inputs
    const [summaryInputRows] = await database_1.db.query('SELECT month, depreciation, income_tax FROM actual_summary_inputs WHERE financial_year = ?', [year]);
    ...
    const depRows = await (0, exports.fetchDepreciation)(year);
    depRows.forEach((row) => {
        MONTHS.forEach(m => {
            if (monthlySummary[m]) {
...
// ==========================================
const fetchDepreciation = async (year) => {
    const [rows] = await database_1.db.query('SELECT * FROM actual_depreciation WHERE financial_year = ?', [year]);
    const grouped = {};
    rows.forEach((row) => {
    ...
    const [hrmsRows] = await database_1.db.query('SELECT * FROM hrms_assets WHERE is_active = 1');
    const MONTHS = ['apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec', 'jan', 'feb', 'mar'];
    hrmsRows.forEach((asset) => {
...
exports.fetchDepreciation = fetchDepreciation;
const upsertDepreciation = async (year, data) => {
    const connection = await database_1.db.getConnection();
    try {
        await connection.beginTransaction();
        let moduleTotal = 0;
        const incomingKeys = new Set(data.map(row => `${row.category}|||${row.assetName}`));
        const [existingRows] = await connection.query('SELECT DISTINCT category, asset_name FROM actual_depreciation WHERE financial_year = ?', [year]);
        for (const row of existingRows) {
            const key = `${row.category}|||${row.asset_name}`;
            if (!incomingKeys.has(key) && !row.asset_name.endsWith('(HRMS)')) {
                await connection.query(`DELETE FROM actual_depreciation WHERE financial_year = ? AND category = ? AND asset_name = ?`, [year, row.category, row.asset_name]);
            }
        }
        const MONTHS = ['apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec', 'jan', 'feb', 'mar'];
        for (const row of data) {
            ...
            for (const monthKey of MONTHS) {
                ...
                if (!isHrms) {
                    await connection.query(`INSERT INTO actual_depreciation 
           ...
           ON DUPLICATE KEY UPDATE 
             ...
             amount = VALUES(amount)`, [year, row.category, row.assetName, row.depPercentage, row.purchaseDate, row.purchaseValue, row.openingDate, row.wdvOpeningValue, monthKey, amount]);
                }
                assetTotal += amount;
                moduleTotal += amount;
            }
        }
        await connection.query(`INSERT INTO actual_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'Depreciation', 'Grand Total', moduleTotal]);
        await connection.commit();
...
exports.upsertDepreciation = upsertDepreciation;
// ==========================================
// 7. AVAILABLE YEARS
// ==========================================
const getCustomers = async () => {
    const [rows] = await database_1.db.query('SELECT CustomerID as id, COALESCE(MasterCustomerName, Name) as name FROM customer');
    return rows;
};
...
exports.fetchAvailableYears = fetchAvailableYears;
