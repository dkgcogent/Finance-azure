...
exports.fetchAvailableYears = exports.upsertDepreciation = exports.fetchDepreciation = exports.fetchSummary = exports.upsertBankCharges = exports.fetchBankCharges = exports.upsertSalaries = exports.fetchSalaries = exports.upsertCorporateExpenses = exports.fetchCorporateExpenses = exports.upsertRevenueDirectExpenses = exports.fetchRevenueDirectExpenses = void 0;
...
const upsertRevenueDirectExpenses = async (year, groups) => {
    ...
    try {
        ...
        const [existingRows] = await connection.query('SELECT DISTINCT customer, project, location FROM budget_revenue_direct_expenses WHERE financial_year = ?', [year]);
        // 3. Delete groups that are no longer present in incoming data
        for (const row of existingRows) {
            const key = `${row.customer}|||${row.project}|||${row.location}`;
            if (!incomingKeys.has(key)) {
                await connection.query(`DELETE FROM budget_revenue_direct_expenses WHERE financial_year = ? AND customer = ? AND project = ? AND location = ?`, [year, row.customer, row.project, row.location]);
            }
        }
        ...
        for (const month of MONTHS) {
            ...
            await connection.query(`INSERT INTO budget_revenue_direct_expenses_totals 
...
const fetchCorporateExpenses = async (year) => {
    ...
    const [hoSalaries] = await database_1.db.query(`SELECT month, SUM(CAST(amount AS DECIMAL(15,2))) AS total_salary 
...
const upsertCorporateExpenses = async (year, data) => {
    ...
    try {
        ...
        const [existingRows] = await connection.query('SELECT DISTINCT head FROM budget_corporate_expenses WHERE financial_year = ?', [year]);
        // 3. Delete heads that are no longer present in incoming data
        for (const row of existingRows) {
            if (!incomingHeads.has(row.head)) {
                await connection.query(`DELETE FROM budget_corporate_expenses WHERE financial_year = ? AND head = ?`, [year, row.head]);
            }
        }
...
const upsertSalaries = async (year, data) => {
    ...
    try {
        ...
        const [existingRows] = await connection.query('SELECT DISTINCT head, customer, project, location, designation, name_of_employee FROM budget_salaries WHERE financial_year = ?', [year]);
        // 3. Delete salaries that are no longer present in incoming data
        for (const row of existingRows) {
            const key = `${row.head}|||${row.customer}|||${row.project}|||${row.location}|||${row.designation}|||${row.name_of_employee}`;
            if (!incomingKeys.has(key)) {
                await connection.query(`DELETE FROM budget_salaries WHERE financial_year = ? AND head = ? AND customer = ? AND project = ? AND location = ? AND designation = ? AND name_of_employee = ?`, [year, row.head, row.customer, row.project, row.location, row.designation, row.name_of_employee]);
            }
        }
...
const upsertBankCharges = async (year, data) => {
    ...
    try {
        ...
        const incomingHeads = new Set(data.map(row => row.head));
        // 2. Fetch existing heads in DB for this year
        const [existingRows] = await connection.query('SELECT DISTINCT head FROM budget_bank_charges WHERE financial_year = ?', [year]);
        // 3. Delete heads that are no longer present in incoming data
        for (const row of existingRows) {
            if (!incomingHeads.has(row.head)) {
                await connection.query(`DELETE FROM budget_bank_charges WHERE financial_year = ? AND head = ?`, [year, row.head]);
            }
        }
        // 4. Insert or Update incoming heads
        for (const row of data) {
...
const fetchDepreciation = async (year) => {
    const [rows] = await database_1.db.query('SELECT * FROM budget_depreciation WHERE financial_year = ?', [year]);
    const grouped = {};
    rows.forEach((row) => {
...
const upsertDepreciation = async (year, data) => {
    ...
    try {
        ...
        const [existingRows] = await connection.query('SELECT DISTINCT category, asset_name FROM budget_depreciation WHERE financial_year = ?', [year]);
        // 3. Delete assets that are no longer present in incoming data
        for (const row of existingRows) {
            const key = `${row.category}|||${row.asset_name}`;
            if (!incomingKeys.has(key)) {
                await connection.query(`DELETE FROM budget_depreciation WHERE financial_year = ? AND category = ? AND asset_name = ?`, [year, row.category, row.asset_name]);
            }
        }
        ...
        for (const row of data) {
            ...
            for (const monthKey of MONTHS) {
                ...
                await connection.query(`INSERT INTO budget_depreciation 
        ...
        await connection.query(`INSERT INTO budget_totals (financial_year, module_name, head_name, total_amount) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE total_amount = VALUES(total_amount)`, [year, 'Depreciation', 'Grand Total', moduleTotal]);
        await connection.commit();
