...
async function ensureGlobalInvoiceManualDataSchema() {
    ...
    `);
        ...
        try {
            const [cols] = await database_1.db.query("SHOW COLUMNS FROM global_invoice_manual_data WHERE Field = 'invoice_id'");
            if (cols[0] && cols[0].Key === 'PRI') {
                await database_1.db.query("ALTER TABLE global_invoice_manual_data ADD COLUMN id INT AUTO_INCREMENT FIRST, DROP PRIMARY KEY, ADD PRIMARY KEY (id)");
            }
        }
        catch (e) {
            // ignore
        ...
        try {
            await database_1.db.query("ALTER TABLE global_invoice_manual_data MODIFY COLUMN invoice_id INT NULL DEFAULT NULL");
        }
        catch (e) {
            // ignore if already nullable
        }
        const [cols] = await database_1.db.query("SHOW COLUMNS FROM global_invoice_manual_data");
        ...
        for (const col of newCols) {
            if (!existing.includes(col.name)) {
                await database_1.db.query(`ALTER TABLE global_invoice_manual_data ADD COLUMN ${col.name} ${col.type}`);
            }
        }
        // Ensure UNIQUE index on standalone_id
        try {
            const [indexes] = await database_1.db.query("SHOW INDEX FROM global_invoice_manual_data WHERE Key_name = 'idx_standalone_id'");
            if (!indexes || indexes.length === 0) {
                await database_1.db.query("ALTER TABLE global_invoice_manual_data ADD UNIQUE INDEX idx_standalone_id (standalone_id)");
            }
        }
        catch (e) {
            // ignore
...
exports.invoiceService = {
    ...
    getLocations: async () => {
        ...
        const locationList = [];
        const seen = new Set();
        const extractCustomerId = (companyName) => {
            if (!companyName)
                return null;
    ...
    generateReports: async (filters) => {
        ...
        let fallbackCustomerCompanyName = null;
        let fallbackCustomerTypeOfServices = null;
        let fallbackCustomerServiceCode = null;
        let fallbackCustomerDetails = null;
        let coreCustomerName = '';
        try {
            if (customerId) {
                const [customerRows] = await database_1.db.query(`
          SELECT 
            CustomerID,
            Name,
            MasterCustomerName,
        ...
        let fallbackProjectGSTIN = null;
        let fallbackProjectTypeOfBilling = null;
        let fallbackProjectGSTRate = null;
        let fallbackProjectBillingTenure = null;
        let fallbackProjectDetails = null;
        try {
            if (projectId) {
                const [projectRows] = await database_1.db.query(`
          SELECT 
            ProjectID,
            ProjectName,
            ProjectCode,
             ...
             id DESC`, [customerId, customerId, projectId, projectId, stateFilter, stateFilter, stateFilter]);
                ...
                flipkartAnnexureData = Array.from(flipkartMap.values()).map((summary) => {
                    ...
                    if (summary.dailyOdoMap) {
                        summary.dailyOdoMap.forEach((dayData) => {
    ...
    `;
        ...
        const regularRows = rows.map((row) => {
