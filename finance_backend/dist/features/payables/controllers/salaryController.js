...
exports.salaryController = {
    getPendingApprovals: async (req, res) => {
        try {
            const [rows] = await database_1.db.query(`SELECT id, month_str, start_date, end_date, status, sent_date, 
    ...
    updateSheetStatus: async (req, res) => {
        try {
            ...
            const [existing] = await database_1.db.query('SELECT * FROM hrms_payment_sheets WHERE id = ?', [id]);
            if (existing.length === 0) {
                return res.status(404).json({ message: 'Payment sheet not found' });
            }
            const todayStr = new Date().toISOString().split('T')[0];
            if (status === 'APPROVED') {
                await database_1.db.query(`UPDATE hrms_payment_sheets 
           SET status = 'APPROVED', approved_date = ?, notes = COALESCE(?, notes) 
           WHERE id = ?`, [todayStr, notes || null, id]);
            }
            else {
                await database_1.db.query(`UPDATE hrms_payment_sheets 
           SET status = 'REJECTED', notes = COALESCE(?, notes) 
           WHERE id = ?`, [notes || 'Rejected in Final Approvals', id]);
            }
            res.json({
                success: true,
    ...
    getSalarySheet: async (req, res) => {
        try {
            ...
            const [approvedSheets] = await database_1.db.query(`SELECT id, month_str, start_date, end_date, status, approved_date, 
         ...
         ORDER BY id DESC LIMIT 1`, [monthStrPattern, numMonth, numYear, numMonth, numYear]);
            ...
            const mappedRows = sheetData.map((row, index) => ({
    ...
    saveBankPaymentSheet: async (req, res) => {
        try {
            await exports.salaryController.ensureSalaryBankPaymentSheetTable();
    ...
    getBankPaymentSheetBatch: async (req, res) => {
        try {
            ...
            const [rows] = await database_1.db.query(`SELECT * FROM SalaryBankPaymentSheet WHERE batch_id = ? ORDER BY id ASC`, [batchId]);
            res.json(rows);
        }
        catch (error) {
