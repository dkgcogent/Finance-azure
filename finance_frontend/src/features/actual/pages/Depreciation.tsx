...
import { exportDepreciationExcel } from "../utils/generateDepreciationExcel"
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                console.error("Failed to export Depreciation Excel:", err);
              } finally {
                      ...
                      <TableRow key={row.id} className="hover:bg-muted/30 group">
                        <TableCell className="border-r-2 border-slate-300 dark:border-slate-700 p-0 h-8">
                          <select
                            ...
                            className="w-full h-full bg-[#e6f2ff] dark:bg-blue-900/40 border-none rounded-none px-2 text-xs shadow-none outline-none focus-visible:ring-1 disabled:opacity-80"
                          >
