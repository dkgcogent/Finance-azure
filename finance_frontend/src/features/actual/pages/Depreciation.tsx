...
import { useActualDepreciationQuery, useSaveActualDepreciationMutation } from "../hooks/useActualDepreciation"
...
import { ActualNav } from "../components/ActualNav"
...
export default function Depreciation() {
  ...
  const { data: serverData, isLoading: isQueryLoading } = useActualDepreciationQuery(selectedYear);
  const { mutateAsync: saveDepreciation, isPending: isSaving } = useSaveActualDepreciationMutation();
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage asset depreciation and amortization actuals.
          </p>
        </div>
          ...
          <Button variant="outline" size="sm" className="h-8" onClick={() => {
            import('xlsx').then(XLSX => {
              ...
              if (table) {
                ...
                XLSX.writeFile(wb, `Depreciation_actual_${selectedYear}.xlsx`);
              }
            });
          }}>
            ...
            <Table id="export-table" className="border-collapse w-max min-w-full" style={{ tableLayout: 'fixed' }}>
              ...
              <TableBody>
                {currentYearData.map((row) => {
                  ...
                  const bgClass = isHrms ? "bg-[#ffff99] dark:bg-yellow-600/40" : "bg-[#ffe6e6] dark:bg-rose-900/20";
                  ...
                  <TableRow key={row.id} className="hover:bg-muted/30 group">
                    <TableCell className="border-r-2 border-slate-300 dark:border-slate-700 p-0 h-8">
                      {isHrms ? (
                        <div className={`w-full h-full ${bgClass} px-2 text-xs flex items-center font-semibold text-black dark:text-white`}>
                          {row.category}
                        </div>
                      ) : (
                        <select
                    ...
                    <TableCell className="border-r-2 border-slate-300 dark:border-slate-700 p-0 h-8">
                      {isHrms ? (
                        ...
                        <Input
                          ...
                          className={`w-full h-full ${bgClass} border-none rounded-none px-2 text-xs shadow-none focus-visible:ring-1 text-black dark:text-white`}
                        />
                      )}
                    </TableCell>
                    ...
                    <TableCell className="border-r-2 border-slate-300 dark:border-slate-700 p-0 h-8">
                      {isHrms ? (
                        <div className={`w-full h-full ${bgClass} px-2 text-xs flex items-center justify-center font-semibold text-black dark:text-white`}>
                          {row.depPercentage}
                        </div>
                      ) : (
                        <Input
                          ...
                          className={`w-full h-full ${bgClass} text-center border-none rounded-none px-2 text-xs shadow-none focus-visible:ring-1 text-black dark:text-white`}
                        />
                      )}
                    </TableCell>
                    ...
                    <TableCell className="border-r-2 border-slate-300 dark:border-slate-700 p-0 h-8">
                      {isHrms ? (
                        <div className={`w-full h-full ${bgClass} px-2 text-xs flex items-center justify-end font-semibold text-black dark:text-white`}>
                          {row.purchaseValue}
                        </div>
                      ) : (
                        <Input
                          ...
                          className={`w-full h-full ${bgClass} text-right border-none rounded-none px-2 text-xs shadow-none focus-visible:ring-1 text-black dark:text-white`}
                        />
                      )}
                    </TableCell>
                    ...
                    <TableCell className="border-r-2 border-slate-300 dark:border-slate-700 p-0 h-8">
                      {isHrms ? (
                        ...
                        <Input
                          ...
                          className={`w-full h-full ${bgClass} text-right font-semibold border-none rounded-none px-2 text-xs shadow-none focus-visible:ring-1 text-black dark:text-white`}
                        />
                      )}
                    </TableCell>

                    {YEARS.map(y => (
                      <TableCell key={y} className={`border-r-2 border-slate-300 dark:border-slate-700 p-0 h-8 ${bgClass}`}>
                        {isHrms ? (
                          <div className={`w-full h-full ${bgClass} px-1 text-xs flex items-center justify-end font-semibold text-black dark:text-white`}>
                            {row[y] || ""}
                          </div>
                        ) : (
                          <Input
                            ...
                            className={`h-full w-full min-w-0 text-right text-xs ${bgClass} border-none shadow-none focus-visible:ring-1 px-1 rounded-none text-black dark:text-white`}
                          />
                        )}
                      </TableCell>
                    ))}
