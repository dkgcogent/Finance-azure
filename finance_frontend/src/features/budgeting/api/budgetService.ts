...
export const getDepreciation = async (financialYear: string) => {
  const response = await apiClient.get(`/budget/depreciation?year=${financialYear}`);
  return response.data;
};

export const saveDepreciation = async (financialYear: string, data: any[]) => {
  const response = await apiClient.post('/budget/depreciation', { year: financialYear, data });
  return response.data;
};
