    ...
    <div className="flex-1 space-y-6 pb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight flex items-center gap-3">
              {budget.budgetId}
              <Badge variant="success" className="text-sm px-2 py-0.5">{budget.status}</Badge>
            </h2>
            <p className="text-muted-foreground mt-1">
              {budget.customer} • {budget.project}
            </p>
          </div>
        </div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            <div className="space-y-1">
              ...
              <p className="font-medium">{budget.financialYear}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs font-medium text-muted-foreground">Region</p>
              <p className="font-medium">{budget.region}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs font-medium text-muted-foreground">Site & Location</p>
              <p className="font-medium">{budget.site}, {budget.location}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs font-medium text-muted-foreground">Executive in Charge</p>
              <p className="font-medium">{budget.executive}</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs font-medium text-muted-foreground">Last Updated</p>
              <p className="font-medium">{budget.lastUpdated}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(budget.totalBudget)}</div>
            <p className="text-xs text-muted-foreground mt-1">
              For financial year {budget.financialYear}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(budget.actualAmount)}</div>
            <p className="text-xs text-muted-foreground mt-1">
              Current YTD expenditure
            </p>
          </CardContent>
        </Card>
        <Card className={budget.variance < 0 ? "border-destructive/50 bg-destructive/5" : "border-emerald-500/50 bg-emerald-500/5"}>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <div className={`text-3xl font-bold ${budget.variance < 0 ? 'text-destructive' : 'text-emerald-600 dark:text-emerald-400'}`}>
              {budget.variance < 0 ? '-' : '+'}{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(Math.abs(budget.variance))}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Planned budget distribution across months.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyBudgetData}>
                  ...
                  <Area type="step" dataKey="allocated" stroke="hsl(var(--primary))" strokeWidth={2} fillOpacity={1} fill="url(#colorBudget)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Positive values indicate under-budget (savings), negative indicate overspend.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
