...
export default function ImprestApprovals() {
  ...
  const [actionPopup, setActionPopup] = useState<{type: 'approve' | 'reject', recordId: string} | null>(null)
  ...
  const groupedData = useMemo(() => {
    ...
    allRecords.filter(record => record.status === 'Pending').forEach(record => {
  ...
  const handleAction = async (type: 'approve' | 'reject') => {
    ...
    const { recordId } = actionPopup;
    const record = selectedSheet?.records.find(r => r.id == recordId);
    if (!record) return;

    try {
      const newStatus = type === 'approve' ? 'Approved' : 'Rejected';
      const newPassAmount = type === 'approve' ? record.amount : 0;
      ...
      // Update local state for the opened sheet to avoid closing and reopening
      setSelectedSheet(prev => {
        if (!prev) return prev;
        return {
          ...prev,
          records: prev.records.map(r => r.id == recordId ? { ...r, status: newStatus, pass_amount: newPassAmount } : r)
        }
      });
    } catch (error) {
    ...
    <div className="flex-1 pb-8">
      {!selectedSheet ? (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-3xl font-bold tracking-tight">Level-1</h2>
                <p className="text-muted-foreground mt-1">
                  Review and approve generated imprest sheets.
                </p>
              </div>
            </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Imprest Approval Queue</CardTitle>
              <CardDescription>Sign off on employee imprest advances.</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="flex items-center gap-4 bg-white p-5 border border-zinc-200 rounded-xl shadow-sm">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <Badge variant={
                  selectedSheet.row.status === "Approved" ? "success" : 
                  selectedSheet.row.status === "Rejected" ? "destructive" : "default"
                }>
                  {selectedSheet.row.status}
                </Badge>
              </div>
                  ...
                  <tbody>
                    {selectedSheet.records.map(record => (
                      <tr key={record.id} className="border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
                        ...
                        <td className="py-4 px-4 text-right text-zinc-500">0</td>
                        <td className="py-4 px-4 text-right font-medium text-emerald-600">{Number(record.pass_amount) || 0}</td>
                        ...
                        <td className="py-4 px-4 text-center">
                          {record.status === 'Approved' ? (
                            ...
                            <div className="flex justify-center items-center gap-2">
                              <Button size="icon" variant="outline" className="h-8 w-8 text-emerald-600 border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700" onClick={() => setActionPopup({type: 'approve', recordId: record.id})}>
                                <CheckCircle2 className="h-4 w-4" />
                              </Button>
                              <Button size="icon" variant="outline" className="h-8 w-8 text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700" onClick={() => setActionPopup({type: 'reject', recordId: record.id})}>
        ...
        <div className="fixed inset-0 bg-zinc-950/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-lg w-full max-w-sm">
            ...
            <p className="text-zinc-600 mb-6">
              Are you sure you want to {actionPopup.type === 'approve' ? 'approve' : 'reject'} this imprest request?
            </p>
            <div className="flex justify-end gap-3">
              <Button variant="outline" onClick={() => setActionPopup(null)}>Cancel</Button>
              <Button 
                className={actionPopup.type === 'approve' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'}
