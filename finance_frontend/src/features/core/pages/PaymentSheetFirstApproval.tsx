...
export default function PaymentSheetFirstApproval() {
  ...
  const fetchImprests = async () => {
    try {
      const res = await apiClient.get('/imprests?status=Approved')
  ...
  const groupedData = useMemo(() => {
    ...
    allRecords.filter(record => record.status === 'Approved').forEach(record => {
  ...
  const handleAction = async (type: 'approve' | 'reject') => {
    ...
    try {
      const newStatus = type === 'approve' ? 'Level-2 Approved' : 'Rejected';
      ...
      setSelectedSheet(prev => {
        ...
        return {
          ...prev,
          records: prev.records.map(r => r.id == recordId ? { ...r, status: newStatus } : r)
        }
      });
    } catch (error) {
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-3xl font-bold tracking-tight">Level-2</h2>
                <p className="text-muted-foreground mt-1">
                  HO - Ops Head review for auto-generated payment sheets.
                </p>
              </div>
            </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>First Approval Queue</CardTitle>
              <CardDescription>Auto-generated payment sheets pending your review.</CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <tr key={record.id} className="border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
                        ...
                        <td className="py-4 px-4 text-center">
                          {record.status === 'Level-2 Approved' ? (
                            <Badge variant="success">Level-2 Approved</Badge>
